...
| **M5** ✅ | **Log correlation — DONE 2026-07-23.** JSON log parser, rule-table classification, markers on the shared axis, click-to-zoom, and the event filter driving both list and markers. Slow queries and other high-volume classes become `logs.*` series in the metric catalogue. | ✅ Verified on a real 73 MB customer log beside its capture: 278 markers from 150k lines, `logs.slowQuery.p95Ms` charted next to WiredTiger, and the oplog-fetcher timeout landing exactly on the dirty-cache cliff. |
