...
| **M2** ✅ | ▲ **Single-capture viewer — DONE 2026-07-22.** uPlot panel, searchable catalog, brush zoom that re-queries at higher resolution, synced cursor, min/max envelope, gap bands. | ✅ Verified in a real browser on a real capture. 258 KB JS (88 KB gzip). First execution of the OPFS backend — see the fresh-origin bug it exposed. |
| **M3** ✅ | **Editable dashboard — DONE 2026-07-22.** react-grid-layout v2, add/remove/resize/drag, per-panel metric chips, localStorage persistence, deflate-compressed size-capped permalink. | ✅ 38 tests. Predefined 6-panel dashboard that degrades to what the capture has. Permalink carries layout only, never data. |
...
| **M6** ✅ | **Insights — DONE 2026-07-28.** Detectors (2026-07-27): 8 rules as data in `src/insights/rules.ts`, run automatically on load, aggregated per (rule, node), conservative under downsampling by construction. **"Explain this window"** (2026-07-28): brush a window and every metric is ranked by how far it moved against the adjacent stretch, log annotations first, one click to put a row on a chart. One scan pass over the window's chunks, not 5,763 series reads. | ✅ Detectors: silent on a healthy 42 h sharded 8.0 node, and on a 67.7 h 7.0.34 capture with a known dirty-cache incident it reports dirty cache ≥20% for 7h57m over 99 episodes, peak 22.8%. Explain: 5,759 metrics in 48 ms (20-min windows), and on that incident it returns forced eviction, cache waits, `document.returned` 6.95k→330k/s. `npm run checks`, `npm run explain`, `npm run verify:explain`. |
| ✚ **M6.5** ✅ | **Member state + node info — DONE 2026-07-30.** A band per replica-set member above the charts, run-length encoded once from the min/max envelope and clipped to the visible window on every zoom; members nobody loaded are drawn from a peer's heartbeats, since that is the only source of `DOWN`. And an info page comparing every loaded node row by row — host, CPU, RAM, WiredTiger cache, build, ulimits and the effective `mongod` configuration — from the FTDC type-0 metadata document, which is now carried whole in the manifest. | ✅ 16 tests, and a real browser run (`npm run verify:replset`) over the 3-node teaching capture: two real failovers drawn, bands tiling their tracks to 99.6%, click-to-zoom reaching the shared range, 43 fields × 3 nodes with the disagreeing rows flagged, no console errors and no network requests. Both cost zero disk reads on open — the strip's runs and the info page's numbers come from the manifest. |
