...
| **M6** | Insights. ▲ **Detectors DONE 2026-07-27** — 8 rules as data in `src/insights/rules.ts`, run automatically on load over the whole capture, findings aggregated per (rule, node) with episode count and worst stretch. Conservative under downsampling by construction. Still to do: "explain this spike" (brush → rank metrics by baseline deviation + correlated log events). | ✅ Ticket saturation and cache pressure surface automatically: silent on a healthy 42 h sharded 8.0 node, and on the 67.7 h 7.0.34 dirty-cache capture reports dirty cache ≥20% for 6h45m over 174 episodes, peak 22.8%. `npm run checks -- <dir>`. Every rule asserted to resolve on 4.4→8.0. |
