...
| **M1** | ▲ **Decoder, per `docs/ftdc-format.md`.** BigInt64Array undelta, per-column type transform, Timestamp→2 columns, cross-column zero-run state, schema drift merge by path. | Full-matrix equality against the oracle on **all six** fixtures. Not a spot check. |
